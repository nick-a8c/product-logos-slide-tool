# Icon Stage — Project Handoff

This document captures the state of the **Icon Stage** project as of v1.0 so a fresh Claude session can pick up from here without re-deriving context. Read it top to bottom before making changes.

## What this project is

A single-file HTML tool that animates a row of brand icons appearing center-outward and exports the result as MP4, WebM, GIF, animated SVG, or PNG. Built for a motion designer at Automattic (Nebojsa) for use during Radical Speed Month and beyond. The whole thing lives in **one HTML file** with all dependencies inlined — drop it on any disk, double-click, it works offline.

The deliverable is `icon-stage.html` (~190 KB). Plain HTML/CSS/JS, no build step, no framework.
...
/home/claude/icon-tool/
  icon-stage.html       # The single deliverable. All code, all deps, all SVGs.
  HANDOFF.md            # This doc.
```

Outputs go to `/mnt/user-data/outputs/icon-stage.html` and are surfaced via `present_files` after each iteration.
...
`state` is a single global object holding everything. Persisted to `localStorage` under key `icon-stage-v2`. Key fields:

- `count`, `spacing`, `scale`, `stageZoom` — composition
...
`buildFilename(ext, extra?)` → `icon-stage[_extra]_<resolution>_<quality>-q_v1.<ext>`. Examples:
...
V1.0. All export formats work offline. Auto mode for all 7 controllable parameters. Quality presets (low/medium/high) for video + GIF. Light/dark theme. Six aspect ratios. Dock/undock/hide panel UX. Top and bottom bars track stage area. Default 9 brand icons embedded. Mirror-staggered animation with proportional slide distance. Descriptive filenames. localStorage persistence with migrations. ~190 KB single-file HTML.

## What's pending or open

- **GitHub repo packaging** is in progress (separate workstream — see `README.md`, `LICENSE`, `.github/workflows/pages.yml` if started)
- **Possible future work**: more anchor points in `AUTO_ANCHORS` for finer interpolation; user presets / sharing via URL hash; hosted version on GH Pages
...
```
cp /home/claude/icon-tool/icon-stage.html /mnt/user-data/outputs/icon-stage.html
```
...
node -e "
const fs = require('fs');
const html = fs.readFileSync('/home/claude/icon-tool/icon-stage.html', 'utf8');
...
```bash
grep -n "function exportMP4\|setPanelState" /home/claude/icon-tool/icon-stage.html
```
